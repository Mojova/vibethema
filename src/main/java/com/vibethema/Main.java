...
public class Main extends Application {
    ...
    private void showSplashScreen(Stage mainStage) {
        ...
        javafx.scene.image.Image splashImg =
                new javafx.scene.image.Image(getClass().getResourceAsStream("/splash.jpg"));
        ...
        splashStage
                .getIcons()
                .add(new javafx.scene.image.Image(getClass().getResourceAsStream("/icon.png")));
        splashStage.show();
        ...
        Task<Scene> loadTask =
                new Task<>() {
                    ...
                    protected Scene call() {
                        ...
                        StartScreen startRoot =
                                (StartScreen)
                                        FluentViewLoader.javaView(StartScreen.class)
                                                .load()
                                                .getView();
                    ...
                    mainStage.setOnCloseRequest(
                            ev -> {
                                if (mainScene.getRoot() instanceof MainView view) {
                                    if (!view.confirmDiscardChanges()) ev.consume();
                                }
                            });
    ...
    private void loadAndStart(Stage stage, File file) {
        if (!new com.vibethema.service.SystemDataService().isCoreDataImported()) {
            ...
            alert.setContentText(
                    "You must import the Exalted 3rd Edition Core PDF before loading characters.");
            alert.showAndWait();
            pendingFile = null;
            start(stage);
            return;
        ...
        try {
            ViewTuple<MainView, MainViewModel> viewTuple =
                    FluentViewLoader.javaView(MainView.class).load();
