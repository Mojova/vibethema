...
public class AbilityRowView extends HBox {
    ...
    public AbilityRowView(AbilityRowViewModel viewModel) {
        ...
        casteLabel.visibleProperty().bind(Bindings.and(viewModel.modeProperty().isEqualTo(CharacterMode.EXPERIENCED), viewModel.isCaste()));
        ...
        casteContainer.managedProperty().bind(viewModel.modeProperty().isEqualTo(CharacterMode.CREATION));
        ...
        favoredBox.visibleProperty().bind(viewModel.modeProperty().isEqualTo(CharacterMode.CREATION));
        favoredLabel.visibleProperty().bind(Bindings.and(viewModel.modeProperty().isEqualTo(CharacterMode.EXPERIENCED), viewModel.isFavored()));
        ...
        favoredContainer.managedProperty().bind(viewModel.modeProperty().isEqualTo(CharacterMode.CREATION));
        ...
        expModeContainer.managedProperty().bind(viewModel.modeProperty().isEqualTo(CharacterMode.EXPERIENCED));
        expModeContainer.visibleProperty().bind(expModeContainer.managedProperty());
        
        // 3. Ability Name Label
        nameLabel = new Label();
        nameLabel.setPrefWidth(95);
        nameLabel.textProperty().bind(Bindings.when(viewModel.isExcellencyAvailableProperty())
        ...
        // Excellency style
        viewModel.isExcellencyAvailableProperty().addListener((obs, oldV, newV) -> updateExcellencyStyle(newV));
    ...
    private void updateExcellencyStyle(boolean isExcellency) {
        if (isExcellency) {
            if (!nameLabel.getStyleClass().contains("excellency-label")) nameLabel.getStyleClass().add("excellency-label");
        } else {
