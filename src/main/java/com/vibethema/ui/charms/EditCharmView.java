...
public class EditCharmView extends GridPane implements JavaView<EditCharmViewModel>, Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        minAb.focusedProperty().addListener((obs, oldV, newV) -> { if (!newV) minAb.increment(0); });
        minEss.focusedProperty().addListener((obs, oldV, newV) -> { if (!newV) minEss.increment(0); });
        ...
        keywordsList.getSelectionModel().getSelectedItems().addListener((javafx.collections.ListChangeListener<String>) c -> {
            viewModel.getSelectedKeywords().setAll(keywordsList.getSelectionModel().getSelectedItems());
        });
        ...
        prereqList.setCellFactory(lv -> new ListCell<>() {
            @Override
        ...
        prereqList.getSelectionModel().getSelectedItems().addListener((javafx.collections.ListChangeListener<Charm>) change -> {
            viewModel.getSelectedPrerequisites().setAll(prereqList.getSelectionModel().getSelectedItems());
        });

        // Re-sync when available items change (e.g. ability change)
        viewModel.getAvailablePrerequisites().addListener((javafx.collections.ListChangeListener<Charm>) change -> {
        ...
        add(new Label("Ability (Group):"), 0, 1); add(abCombo, 1, 1);
        add(new Label("Cost:"), 0, 2); add(costField, 1, 2);
        add(new Label("Min Essence:"), 0, 3); add(minEss, 1, 3);
        add(new Label("Min Ability:"), 0, 4); add(minAb, 1, 4);
        add(new Label("Type:"), 0, 5); add(typeField, 1, 5);
        add(new Label("Keywords:"), 0, 6); add(keywordsList, 1, 6);
        add(new Label("Duration:"), 0, 7); add(durationField, 1, 7);
        add(new Label("Prerequisites:"), 0, 8); add(prereqList, 1, 8);
        add(new Label("Description:"), 0, 9); add(descArea, 1, 9);
