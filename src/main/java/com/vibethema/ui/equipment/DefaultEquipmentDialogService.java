...
public class DefaultEquipmentDialogService implements EquipmentDialogService {
    ...
    public Optional<Weapon> showWeaponDialog(
        ...
        EquipmentDataService svc =
                (equipmentService != null) ? equipmentService : this.equipmentService;
        ...
        WeaponDialogViewModel viewModel =
                new WeaponDialogViewModel(existing, allTags, characterSpecialties);

        de.saxsys.mvvmfx.ViewTuple<WeaponDialogView, WeaponDialogViewModel> viewTuple =
                de.saxsys.mvvmfx.FluentViewLoader.javaView(WeaponDialogView.class)
                        .viewModel(viewModel)
                        .load();
        ...
        ButtonType saveBtnType =
                new ButtonType(existing == null ? "Add" : "Save", ButtonBar.ButtonData.OK_DONE);
    ...
    @Override
    public Optional<Armor> showArmorDialog(
        ...
        ViewTuple<ArmorDialogView, ArmorDialogViewModel> vt =
                FluentViewLoader.javaView(ArmorDialogView.class).viewModel(viewModel).load();
                ...
                b -> {
                    if (b == saveBtn) {
                        Hearthstone h =
                                (existing != null)
                                        ? existing
                                        : new Hearthstone(name.getText(), description.getText());
    ...
    @Override
    public Optional<OtherEquipment> showOtherEquipmentDialog(
            OtherEquipment existing, Window owner) {
        ...
        Spinner<Integer> attunement =
                new Spinner<>(0, 20, existing == null ? 0 : existing.getAttunement());
        ...
        grid.add(new Label("Attunement:"), 0, 3);
        grid.add(attunement, 1, 3);
        grid.add(new Label("Attunement:"), 0, 3);
        grid.add(attunement, 1, 3);
                ...
                b -> {
                    if (b == saveBtn) {
                        OtherEquipment oe =
                                (existing != null)
                                        ? existing
                                        : new OtherEquipment(name.getText(), description.getText());
