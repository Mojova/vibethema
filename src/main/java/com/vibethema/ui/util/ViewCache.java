...
public class ViewCache {
    ...
    private static final Map<Class<? extends JavaView<?>>, ViewTuple<?, ?>> cache = new ConcurrentHashMap<>();

    /**
     * Pre-loads a view in the current thread (should be called from a background thread or at startup if safe).
     * Note: MVVMFX FluentViewLoader might have UI-thread requirements for certain components.
     */
    ...
    /**
     * Gets a cached ViewTuple or loads a new one if not cached.
     * The returned instance is removed from the cache to ensure ownership.
     */
    @SuppressWarnings("unchecked")
    public static <V extends JavaView<VM>, VM extends ViewModel> ViewTuple<V, VM> get(Class<V> viewType) {
        ViewTuple<V, VM> tuple = (ViewTuple<V, VM>) cache.remove(viewType);
        if (tuple == null) {
            logger.debug("Cache miss for view: {}, loading synchronously", viewType.getSimpleName());
