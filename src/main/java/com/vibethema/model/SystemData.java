...
/**
 * Centralized registry for system-wide data, rules, and metadata that aren't specific to a single
 * character.
 */
public class SystemData {
    public static final List<Attribute> PHYSICAL_ATTRIBUTES =
            Arrays.stream(Attribute.values())
                    .filter(a -> a.getCategory() == Attribute.Category.PHYSICAL)
                    .collect(Collectors.toList());

    public static final List<Attribute> SOCIAL_ATTRIBUTES =
            Arrays.stream(Attribute.values())
                    .filter(a -> a.getCategory() == Attribute.Category.SOCIAL)
                    .collect(Collectors.toList());

    public static final List<Attribute> MENTAL_ATTRIBUTES =
            Arrays.stream(Attribute.values())
