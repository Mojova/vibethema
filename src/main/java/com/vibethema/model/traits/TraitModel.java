...
public class TraitModel {
    private final ObservableMap<Attribute, IntegerProperty> attributes = FXCollections.observableHashMap();
    private final ObservableMap<Ability, IntegerProperty> abilities = FXCollections.observableHashMap();
    private final ObservableMap<Ability, BooleanProperty> casteAbilities = FXCollections.observableHashMap();
    private final ObservableMap<Ability, BooleanProperty> favoredAbilities = FXCollections.observableHashMap();
    private final ObservableMap<Attribute.Category, ObjectProperty<AttributePriority>> attributePriorities = FXCollections.observableHashMap();
    ...
    public TraitModel(Consumer<Void> dirtyListener, Runnable statUpdateTrigger) {
        ...
        for (Attribute attr : SystemData.ATTRIBUTES) {
            ...
            attributes.get(attr).addListener((obs, oldV, newV) -> {
                markDirty();
        ...
        for (Ability ability : SystemData.ABILITIES) {
            ...
            abilities.get(ability).addListener((obs, oldV, newV) -> {
                markDirty();
                statUpdateTrigger.run();
            });
            ...
            casteAbilities.get(ability).addListener((obs, oldV, newV) -> {
                ...
                if (!isImporting && (ability == Ability.BRAWL || ability == Ability.MARTIAL_ARTS)) {
                    isImporting = true;
                    try {
                        Ability other = (ability == Ability.BRAWL) ? Ability.MARTIAL_ARTS : Ability.BRAWL;
        ...
        merits.addListener((javafx.collections.ListChangeListener<? super Merit>) c -> {
            markDirty();
            statUpdateTrigger.run();
        });
    ...
    public ObservableMap<Ability, BooleanProperty> getCasteAbilities() { return casteAbilities; }
    public ObservableMap<Ability, BooleanProperty> getFavoredAbilities() { return favoredAbilities; }
