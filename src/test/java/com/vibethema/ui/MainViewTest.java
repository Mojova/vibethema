...
public class MainViewTest {
    ...
    void testLazyLoadingTabs() throws InterruptedException {
        ...
        Platform.runLater(() -> {
            try {
                ...
                // We load MainView. Note: in real app we use ViewCache, but here we load fresh for testing.
                ...
                // Nest multiple Platform.runLater calls to match StatsTab's staggered loading (4 pulses)
                Platform.runLater(() -> {
                    Platform.runLater(() -> {
                        Platform.runLater(() -> {
                            Platform.runLater(() -> {
                                ...
                                assertNotNull(content, "Stats tab should eventually load content");
                                // Verify at least one section was added
                                assertFalse(content.getChildren().isEmpty(), "Stats content should not be empty");
    ...
    void testStaggeredStatsLoad() throws InterruptedException {
        ...
        Platform.runLater(() -> {
            try {
                CharacterData data = new CharacterData();
                StatsTab statsTab = (StatsTab) de.saxsys.mvvmfx.FluentViewLoader
                        .javaView(StatsTab.class)
                        .viewModel(new com.vibethema.viewmodel.StatsViewModel(data))
                        .load().getView();
                ...
                Platform.runLater(() -> {
                    ...
                    VBox content = (VBox) scrollPane.getContent();
                    assertNotNull(content, "Content should be initialized in first pulse");
                    assertEquals(1, content.getChildren().size(), "Should have 1 section (Advantages)");

                    // Pulse 2: Attributes (Step 2)
                    Platform.runLater(() -> {
                        assertEquals(2, content.getChildren().size(), "Should have 2 sections (Advantages + Attributes)");

                        // Pulse 3: Abilities (Step 3)
                        Platform.runLater(() -> {
                            // Plus 1 for the separator added in step 3
                            assertEquals(4, content.getChildren().size(), "Should have added Abilities and Separator");

                            // Pulse 4: Final stats (Step 4)
                            Platform.runLater(() -> {
                                assertEquals(7, content.getChildren().size(), "Should have added all remaining sections");
                                latch.countDown();
                            });
                        });
                    });
