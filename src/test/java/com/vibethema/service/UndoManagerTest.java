...
class UndoManagerTest {
    ...
    void testBasicUndoRedo() {
        ...
        // Push state 1
        undoManager.pushCheckpoint(state1, "TAB1", "Action 1");
        assertTrue(undoManager.canUndoProperty().get());
        assertFalse(undoManager.canRedoProperty().get());

        // Push state 2
        undoManager.pushCheckpoint(state2, "TAB1", "Action 2");
        
        // Push state 3
        undoManager.pushCheckpoint(state3, "TAB1", "Action 3");
    ...
    void testContextFiltering() {
        CharacterSaveState state1 = new CharacterSaveState();
        undoManager.pushCheckpoint(state1, "TAB_STATS", "Change Strength");
    ...
    void testIdenticalStateIgnored() {
        ...
        undoManager.pushCheckpoint(state1, "CONTEXT", "Action 1");
        
        CharacterSaveState state2 = new CharacterSaveState();
        state2.name = "Name"; // Identical
        
        undoManager.pushCheckpoint(state2, "CONTEXT", "Action 2");
    ...
    void testRedoStackClearedOnNewAction() {
        CharacterSaveState state1 = new CharacterSaveState();
        undoManager.pushCheckpoint(state1, "C1", "A1");
        undoManager.undo("C1", state1);
        assertTrue(undoManager.canRedoProperty().get());

        undoManager.pushCheckpoint(new CharacterSaveState(), "C1", "A2");
        assertFalse(undoManager.canRedoProperty().get());
    }
}
