...
public class FinalizationTest {
    ...
    void testReadyForFinalize() {
        ...
        data.getAttribute(Attribute.WITS).set(1); // Sum: (3+3+2) + (2+2+2) + (2+1+0) = 8 + 6 + 3 = 17. 
        ...
        data.getAbility(SystemData.ABILITIES.get(10)).set(3); // +3 dots. Non-favored cost is 2 BP per dot. 3*2 = 6 BP.
        // Total so far: 4 + 6 = 10 BP.
        // Raising a 12th ability to 2 (favored) costs 1 BP per dot. +5.
        data.getAbility(SystemData.ABILITIES.get(11)).set(5); // +5 dots. Favored cost is 1 BP per dot. Total pool is now > 28.
        ...
        assertTrue(status.isReadyToFinalize, "Character should be ready to finalize when all points are spent");
    }
    ...
    void testNotReadyIfUnderPoints() {
        ...
        for (int i = 0; i < 15; i++) data.getUnlockedCharms().add(new PurchasedCharm("C"+i, "Charm "+i, "Melee")); // 15 charms
